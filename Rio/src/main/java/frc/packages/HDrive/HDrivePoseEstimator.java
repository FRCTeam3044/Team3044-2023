...
public class HDrivePoseEstimator {
  ...
  private final HDriveOdometry m_odometry;
   ...
   * @param trackWidthMeters The track width of the robot.
   * @param leftDistanceMeters The distance traveled by the left encoder.
   * @param rightDistanceMeters The distance traveled by the right encoder.
   * @param lateralDistanceMeters The distance traveled by the lateral encoder.
   ...
   * @param leftPositionMeters The distance traveled bm_poseBuffery the left encoder.
   * @param rightPositionMeters The distance traveled by the right encoder.
   * @param lateralPositionMeters The distance traveled by the lateral encoder.
   ...
   * HDrivePoseEstimator#update} every loop.
   *
   ...
   * HDrivePoseEstimator#setVisionMeasurementStdDevs(Matrix)} or this method.
   *
   * @param visionRobotPoseMeters The pose of the robot as measured by the vision camera.
   * @param timestampSeconds The timestamp of the vision measurement in seconds. Note that if you
   *     don't use your own time source by calling {@link
   ...
   * @param distanceLateralMeters The total distance travelled by the lateral wheel in meters.
  ...
  public Pose2d updateWithTime(
    ...
    m_odometry.update(gyroAngle, distanceLeftMeters, distanceRightMeters, distanceLateralMeters);
  ...
  private class InterpolationRecord implements Interpolatable<InterpolationRecord> {
    ...
    // The distance traveled by the lateral encoder.
    private final double lateralMeters;
    ...
    public InterpolationRecord interpolate(InterpolationRecord endValue, double t) {
      if (t < 0) {
        ...
        // Find the new lateral distance.
        var lateral_lerp = MathUtil.interpolate(this.lateralMeters, endValue.lateralMeters, t);
    ...
    public boolean equals(Object obj) {
      ...
      return Objects.equals(gyroAngle, record.gyroAngle)
          ...
          && Double.compare(lateralMeters, record.lateralMeters) == 0
          && Objects.equals(poseMeters, record.poseMeters);
    }

    @Override
    public int hashCode() {
      return Objects.hash(gyroAngle, leftMeters, rightMeters, lateralMeters, poseMeters);
    }
  }
}