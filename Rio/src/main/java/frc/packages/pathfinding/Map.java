...
public class Map {
    ...
    private void lineOfSight(int cur, int i, ArrayList<Edge> neighborArray, ArrayList<Node> pathVerticiesArray) {
        ...
        for (Edge obstacleEdge : obstacleEdges) {
            if (Vector.dotIntersect(pathVerticiesArray.get(cur), pathVerticiesArray.get(i), obstacleVerticies.get(obstacleEdge.vertexOne), obstacleVerticies.get(obstacleEdge.vertexTwo))) {
                intersect = true;
                break;
            }
        }
