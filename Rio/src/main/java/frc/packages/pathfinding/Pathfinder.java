...
import frc.packages.pathfinding.Structures.Edge;
import frc.packages.pathfinding.Structures.ImpossiblePathException;
import frc.packages.pathfinding.Structures.Node;
import frc.packages.pathfinding.Structures.Obstacle;
...
public class Pathfinder
{
     ...
     */
    public Path generatePath (Node start, Node target, PathfindSnapMode snapMode, ArrayList<Node> dynamicVerticies) throws ImpossiblePathException
    {
        long snapStartTime = System.nanoTime();
        // Snapping start and target to be outside of obstacles
        start = Node.fromVertex(snap(start, false));
        ...
        if(snapMode == PathfindSnapMode.SNAP || snapMode == PathfindSnapMode.SNAP_THEN_POINT || snapToScoringNodes){
            target = Node.fromVertex(snap(unsnappedTarget, snapToScoringNodes));
            System.out.println("Snapped target (iter 1): " + target.print());
            if(snapToScoringNodes){
                unsnappedTarget = target;
                target = Node.fromVertex(snap(unsnappedTarget, false));
    ...
    public Vertex snap(Vertex point, boolean snapToScoringNodes){
        ArrayList<Obstacle> targetObs = Obstacle.isRobotInObstacle(obstacles, point, snapToScoringNodes);
        if(targetObs.isEmpty()) return point;
        Vertex tempNearestNode = point;
        for(Obstacle obs : targetObs){
            tempNearestNode = obs.calculateNearestPoint(tempNearestNode, snapToScoringNodes);
        }
